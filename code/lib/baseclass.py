...
class BaseClass():
    ...
    def set_presets(self,os_environ):
        ...
        self.presets.update( { "learning_rate" : float(os_environ.get("LEARNING_RATE")) if "LEARNING_RATE" in os_environ else [ 1e-4 ] } )
        self.presets.update( { "batch_size" : json.loads(os_environ.get("BATCH_SIZE")) if "BATCH_SIZE" in os_environ else 64 } )
